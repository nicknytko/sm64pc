...
void update_mario_button_inputs(struct MarioState *m) {
    if (m->controller->pc_controls->jump_pressed) {
        m->input |= INPUT_A_PRESSED;
    }

    if (m->controller->pc_controls->jump) {
        m->input |= INPUT_A_DOWN;
    }

    // Don't update for these buttons if squished.
    if (m->squishTimer == 0) {
        if (m->controller->pc_controls->punch_pressed ||
            m->controller->pc_controls->mouse_left_pressed) {

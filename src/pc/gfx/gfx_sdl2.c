...
static void gfx_sdl_handle_events(void) {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_MOUSEBUTTONDOWN:
            val = (event.button.type == SDL_MOUSEBUTTONDOWN ? 1 : 0);
            index = (event.button.button == SDL_BUTTON_LEFT ? 0 : 1);
            mouse_click(index, val);
            break;
        case SDL_MOUSEWHEEL:
            up = event.wheel.y > 0;
            down = event.wheel.y < 0;
            mouse_wheel(up, down);
            break;
#endif
        case SDL_QUIT:
